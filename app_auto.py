            ...
            for _ in range(5):
                ...
                if response.status_code == 200:
                    ...
                    msg_placeholder.text(
                        f"Now Playing  Track: {track_info['name']} Artist: {', '.join(artist['name'] for artist in track_info['artists'])}"
                    )
...
def main():
    ...
    if choice == "Home":
        ...
        #     if detected_emotion:
        #         selected_track_uri = music_player.recommend_music(detected_emotion,generic_mood)
        #         if selected_track_uri:
        #             track_info = sp.track(selected_track_uri)
        #             preview_url = track_info["preview_url"]
        #
        #             if preview_url:
        #                 st.write(f"Now Playing {preview_url}")
        #                 audio_html = f"""
        #                                 <audio controls autoplay onended="playNextSong()">
        #                                     <source src="{preview_url}" type="audio/mpeg">
        #                                     Your browser does not support the audio element.
        #                                 </audio>
        #                                 <script>
        #                                     function playNextSong() {{
        #                                         window.location.reload();  // Reload the page to play the next song
        #                                     }}
        #                                 </script>
        #                              """
